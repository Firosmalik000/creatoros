...
import { getCreatorOrderDetail } from "@/lib/order-server";
...
export default async function CreatorOrderDetailPage({ params }: PageProps) {
  ...
  if (!cookieStore.get("creatoros_session")) {
    redirect(
      `/${locale}/auth/login?redirect=/${locale}/creator/orders/${orderID}`,
    );
  }

  const [detail, workflow, thread] = await Promise.all([
  ...
  const [t, wt, ct] = await Promise.all([
  ...
  return (
    <div className="space-y-5">
      ...
      >
        <ArrowLeft aria-hidden="true" size={16} /> {t("backToCreatorOrders")}
      </Link>
        ...
        <header className="order-detail-header">
          <div>
            ...
            <p className="order-detail-header__creator">
              {t("client")}: <strong>{detail.client_display_name}</strong>
            </p>
          </div>
          ...
          <div className="order-detail-main">
            <OrderDetailTabs
              ...
              discussionContent={
                <OrderConversation
                  orderID={detail.id}
                  currentUserID={detail.creator_user_id}

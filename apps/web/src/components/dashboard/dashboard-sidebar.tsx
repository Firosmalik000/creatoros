...
export function DashboardSidebar({
  ...
  const roleBadges: Record<string, { label: string; icon: React.ComponentType<{ size?: number }>; color: string }> = {
    admin: { label: "Agency Admin", icon: ShieldCheck, color: "bg-amber-500/10 text-amber-400 border-amber-500/20" },
    creator: { label: "Creator Studio", icon: Palette, color: "bg-purple-500/10 text-purple-400 border-purple-500/20" },
    client: { label: "Brand Hub", icon: Building2, color: "bg-blue-500/10 text-blue-400 border-blue-500/20" },
  };
  ...
  return (
    <>
      ...
      {/* Sidebar Container */}
      <aside
        className={`fixed top-0 bottom-0 left-0 z-50 w-72 bg-[#090d16] border-r border-white/10 flex flex-col transition-transform duration-300 ease-in-out lg:translate-x-0 ${
      ...
      >
        ...
        <div className="h-16 px-6 border-b border-white/10 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            >
              <span>Creator</span>
              <span className="text-[#3b82f6]">OS</span>
            </Link>
            <span
        ...
        <nav className="flex-1 overflow-y-auto px-4 py-6 space-y-6 custom-scrollbar">
          {groups.map((group, groupIdx) => (
            <div key={groupIdx} className="space-y-1.5">
              <h3 className="px-3 text-[11px] font-semibold uppercase tracking-wider text-white/40 mb-2">
                {group.title}
              </h3>
                  ...
                  return (
                    <li key={item.href}>
                      <Link
                        href={item.href}
                        onClick={onCloseMobile}
                        className={`group flex items-center justify-between px-3 py-2 rounded-xl text-sm font-medium transition-all duration-150 ${
                          isActive
        ...
        <div className="p-4 border-t border-white/10 bg-white/[0.01] space-y-3">
          <Link
            ...
            className="flex items-center justify-between p-2.5 rounded-xl text-xs font-medium text-white/70 hover:text-white hover:bg-white/[0.04] transition-colors border border-white/5"
          >
            ...
            <Settings size={15} className="text-white/40 hover:text-white" />
          </Link>

          <div className="flex items-center justify-between px-2 pt-1">
            <Link
              href={`/${locale}`}
              className="text-[11px] font-medium text-white/40 hover:text-white flex items-center gap-1 transition-colors"
            >
              Marketplace
            ...
            <Link
              ...
              className="text-[11px] font-medium text-red-400/70 hover:text-red-400 flex items-center gap-1 transition-colors"
            >
              <LogOut size={12} />
              Exit
            </Link>
